...
use crate::{colliders::*, math::Mat4Ext, contacts::Intersection};
...
fn epa_expand(
    ...
    loop {
        ...
        let new_pt = support(collider_a, trans_a, collider_b, trans_b, normal, bias);
    ...
    let delta =  pt_on_a - pt_on_b;
    
    // (pt_on_a, pt_on_b)
    Some(Intersection {
        normal: delta.normalize_or_zero(),
        penetration: delta.length()
    })
}
